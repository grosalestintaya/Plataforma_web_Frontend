...
export default function GradeStudents() {
  ...
  const fetchStudents = async () => {
    ...
    try {
      const json = await TeacherService.listStudents({ q, id_grade, page, limit });
  ...
  return (
    <div ref={containerRef} className="space-y-5" style={{ background: tokens.appBg }}>
      ...
      <div className="space-y-5 p-6 rounded-3xl border shadow-sm" style={{ background: "#fff", borderColor: tokens.cardBorder }}>
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          ...
          <div className="md:flex md:items-center md:gap-4">
            <Link
              ...
              style={{ background: tokens.dashBg, borderColor: tokens.cardBorder, color: tokens.dashText }}
            >
              ← Volver
            </Link>
            ...
            <div
              ...
              style={{ background: "#fff", borderColor: tokens.cardBorder, color: "rgba(100,116,139,0.95)" }}
            >
              Total:{" "}
              <span className="font-extrabold" style={{ color: tokens.dashText }}>
                {pagination.total ?? 0}
              </span>
            </div>
          </div>
        ...
        <div className="rounded-3xl p-4 border" style={{ background: "#fff", borderColor: tokens.cardBorder }}>
          <div className="grid gap-3 md:grid-cols-12 md:items-end">
            <div className="md:col-span-9">
              <label className="text-xs font-extrabold" style={{ color: "rgba(100,116,139,0.95)" }}>
                Buscar
              </label>
              <input
                value={q}
        ...
        <div className="rounded-3xl p-4 border" style={{ background: "#fff", borderColor: tokens.cardBorder }}>
          {loading && (
            <div className="grid gap-3 md:grid-cols-3">
              {Array.from({ length: Math.min(limit, 9) }).map((_, i) => (
                ...
                >
                  <div className="h-4 w-2/3 rounded" style={{ background: "rgba(15,23,42,0.14)" }} />
                  <div className="mt-3 h-3 w-1/2 rounded" style={{ background: "rgba(15,23,42,0.12)" }} />
                  <div className="mt-4 h-9 w-full rounded-2xl" style={{ background: "rgba(15,23,42,0.12)" }} />
                </div>
              ))}
            </div>
          )}
          ...
          {!loading && !error && items.length > 0 && (
            <>
              ...
              <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t pt-4" style={{ borderColor: tokens.cardBorder }}>
                <div className="text-sm font-semibold" style={{ color: "rgba(100,116,139,0.95)" }}>
                  Página{" "}
                  <span className="font-extrabold" style={{ color: tokens.dashText }}>
                    {pagination.page}
                  </span>{" "}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-sm font-extrabold" style={{ color: tokens.dashText }}>
            {student.full_name}
          </div>
