...
export default function StudentDetails() {
  ...
  const fetchDetail = async () => {
    ...
    try {
      const json = await TeacherStudentsService.getDetailById(id_user);
      setPayload(json ?? null);
    } catch (e) {
      // 401 lo maneja apiClient
      if (e?.status !== 401) setError(e?.message || "Error al cargar el detalle del estudiante");
      setPayload(null);
    } finally {
      setLoading(false);
    }
      ...
      <div className="space-y-5 bg-white p-6 rounded-3xl ring-1 ring-slate-200">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link
              ...
              className="rounded-xl bg-[#00C853] px-3 py-2 text-sm font-extrabold text-white hover:opacity-90"
            >
              ← Volver
            </Link>
            ...
            <button
              onClick={fetchDetail}
              className="rounded-2xl bg-slate-100 px-4 py-2 text-sm font-extrabold text-slate-800 ring-1 ring-slate-200 hover:bg-slate-50"
              type="button"
            >
              Actualizar
            </button>
        ...
        {!loading && !error && payload && (
          <>
            ...
            {/* Student info (sin IDs internos; DNI masked) */}
              ...
              <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
                    ...
                    <div className="mt-2 text-sm text-slate-700">
                      Ha completado{" "}
                      <span className="font-extrabold">{overview?.activitiesCompleted ?? 0}</span> de{" "}
                      <span className="font-extrabold">{overview?.activitiesTotal ?? 0}</span> actividades.
                    </div>
                  ...
                  <div className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
                    ...
                    <div className="mt-2 text-sm text-slate-700">
                      Intentos totales: <span className="font-extrabold">{overview?.attemptsTotal ?? 0}</span>
                    </div>
                    <div className="mt-2 text-sm text-slate-700">
                      Recompensas: <span className="font-extrabold">{overview?.xpTotal ?? 0} XP</span> y{" "}
            ...
            {tab === "actividades" && (
              <div className="space-y-4">
                {modules.map((m) => (
                  <div
                    key={`${m.id_module}-${m.module_sort_order}`}
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        ...
                        <div className="mt-1 text-xs font-semibold text-slate-600">{m.items.length} actividad(es)</div>
                      </div>
            ...
            {tab === "respuestas" && (
              <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <div className="text-sm font-extrabold text-slate-900">Últimos intentos registrados</div>
                ...
                <div className="mt-4 space-y-3">
                  {lastAttemptActivities.length === 0 && (
                    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-700">
                      ...
                      <div className="mt-1">Este estudiante aún no tiene intentos con respuestas registradas.</div>
                    </div>
                  )}
...
{JSON.stringify(a.last_payload, null, 2)}
            ...
            {tab === "insignias" && (
              <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
                ...
                <div className="mt-1 text-sm text-slate-600">Insignias obtenidas por el estudiante.</div>
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      {insignias.map((b, idx) => (
                        <div key={idx} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                          <div className="text-sm font-extrabold text-slate-900">{b?.name ?? "Insignia"}</div>
                          {b?.description && <div className="mt-1 text-sm text-slate-700">{b.description}</div>}
                        </div>
                      ))}
                    </div>
                  )}
...
function KpiCard({ label, value, hint, accent, dark }) {
  return (
    <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
      ...
      <div className={`mt-2 text-2xl font-extrabold ${accent ?? "text-slate-900"}`}>{value}</div>
      {hint && (
        <div className={`mt-1 text-xs font-semibold ${dark ? "text-slate-800" : "text-slate-600"}`}>{hint}</div>
      )}
    </div>
  );
}
    ...
    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <div className="text-sm font-extrabold text-slate-900">{a.best_score != null ? a.best_score : "—"}</div>
        </div>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-3">
        <MiniStat label="Intentos" value={a.attempts_count ?? 0} />
        <MiniStat label="Último intento" value={a.last_attempt_at ? formatDateShort(a.last_attempt_at) : "—"} />
...
function Pill({ tone = "slate", children }) {
  ...
  return <span className={`rounded-full px-3 py-1 text-xs font-extrabold ring-1 ${map[tone]}`}>{children}</span>;
}
