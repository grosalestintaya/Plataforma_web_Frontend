...
export default function Graficos() {
  ...
  return (
    <div ref={containerRef} className="w-full min-w-0" style={{ background: "var(--app-bg)" }}>
      <div className="p-6">
        <header
        ...
        >
          <div className="text-lg font-semibold" style={{ color: "var(--dash-title-text)" }}>
            Gráficos de Rendimiento y Actividad
          </div>
          <div className="text-xs mt-1" style={{ color: "rgba(100,116,139,0.95)" }}>
            Rango: <span className="font-medium">{data?.rangeDays ?? 30} días</span>
          </div>
        </header>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <Panel title="Score promedio por módulo" subtitle="Mejor score promedio por módulo (0–100)">
                <SafeResponsive height={320} debounce={RC_DEBOUNCE}>
                  <BarChart data={avgScoreByModule} layout="vertical" margin={{ left: 12, right: 16 }}>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ borderRadius: 12, borderColor: colors.border }}
                      labelFormatter={() => ""}
                      formatter={(v, _k, p) => [`${Number(v).toFixed(1)}%`, p?.payload?.module]}
                    />
                    <Bar dataKey="avg" fill={colors.primary} radius={[10, 10, 10, 10]} />
                  </BarChart>
                </SafeResponsive>
              </Panel>

              <Panel title="Top insignias" subtitle="Ranking de insignias otorgadas">
                <SafeResponsive height={320} debounce={RC_DEBOUNCE}>
                  <BarChart data={insigniasTop10} layout="vertical" margin={{ left: 12, right: 16 }}>
                    <CartesianGrid stroke={colors.grid} vertical={false} />
                    <XAxis
                      type="number"
                      tick={{ fill: colors.text, fontSize: 12 }}
                    ...
                    />
                    <Bar dataKey="count" fill={colors.accent} radius={[10, 10, 10, 10]} />
                  </BarChart>
                </SafeResponsive>
              </Panel>
            </div>
            ...
            {/* Fila 2 */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <Panel title="Intentos por día" subtitle="Volumen de intentos (engagement)">
                <SafeResponsive height={300} debounce={RC_DEBOUNCE}>
                  <LineChart data={attemptsByDay} margin={{ left: 8, right: 16 }}>
                    <CartesianGrid stroke={colors.grid} vertical={false} />
                    <XAxis
                      dataKey="d"
                      tick={{ fill: colors.text, fontSize: 12 }}
                    ...
                    <Tooltip
                      ...
                      formatter={(v, _k, p) => [`${v}`, `Intentos · ${p?.payload?.date}`]}
                    />
                    <Line type="monotone" dataKey="attempts" stroke={colors.primary} strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </SafeResponsive>
              </Panel>

              <Panel title="XP por día" subtitle="XP total acumulado por día (gamificación)">
                <SafeResponsive height={300} debounce={RC_DEBOUNCE}>
                  <AreaChart data={xpByDay} margin={{ left: 8, right: 16 }}>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ borderRadius: 12, borderColor: colors.border }}
                      labelFormatter={tooltipDateLabel}
                      formatter={(v, _k, p) => [`${v}`, `XP · ${p?.payload?.date}`]}
                    />
                    <Area
                      type="monotone"
                      dataKey="xp"
            ...
            {/* Fila 3 */}
            <div className="mb-6">
              <Panel title="Aprobados vs Reprobados por día" subtitle="Barras apiladas por fecha + % aprobación">
                <SafeResponsive height={340} debounce={RC_DEBOUNCE}>
                  <BarChart data={passFailByDay} margin={{ left: 8, right: 16 }}>
                    <CartesianGrid stroke={colors.grid} vertical={false} />
                    <XAxis
                      dataKey="d"
                      tick={{ fill: colors.text, fontSize: 12 }}
                    ...
                    <Tooltip
                      ...
                      labelFormatter={tooltipDateLabel}
                      formatter={(v, key, p) => {
                        if (key === "pass") return [`${v}`, `Aprobados · ${p?.payload?.date}`];
                        if (key === "fail") return [`${v}`, `Reprobados · ${p?.payload?.date}`];
                        return [v, key];
                      }}
                    />
                    <Legend />
                    <Bar dataKey="pass" stackId="pf" fill={colors.primary} radius={[10, 10, 0, 0]} />
                    <Bar dataKey="fail" stackId="pf" fill={colors.fail} radius={[0, 0, 10, 10]} />
                  </BarChart>
                </SafeResponsive>
                  ...
                  {passFailByDay.map((d) => (
                    ...
                    >
                      ...
                      <div className="text-[11px]" style={{ color: "rgba(100,116,139,0.95)" }}>
                        Pass: {d.pass} · Fail: {d.fail} · Total: {d.total} · {pct(d.pass_pct)}
                      </div>
                    </div>
                  ))}
                </div>
