...
import { TeacherStudentsService } from "@/features/dashboard/services/teacherStudents.service";
...
export default function StudentDetails() {
  ...
  const modules = useMemo(() => {
    ...
    for (const m of arr) {
      m.items.sort(
        (x, y) => Number(x.activity_sort_order) - Number(y.activity_sort_order),
      );
    }
    return arr;
  }, [activities]);
  ...
  const lastAttemptActivities = useMemo(() => {
    return activities
      .filter(
        (a) => a?.last_payload && Object.keys(a.last_payload || {}).length > 0,
      )
      .sort((a, b) => {
        const da = a?.last_attempt_at
          ? new Date(a.last_attempt_at).getTime()
          : 0;
        const db = b?.last_attempt_at
          ...
          <>
            ...
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <KpiCard
                label="Promedio"
                value={
                  overview?.avgBestScore != null
                    ? `${Math.round(overview.avgBestScore)}`
                    : "—"
                }
                hint="Best score"
                accent="text-[#2962FF]"
              ...
              <KpiCard
                ...
                value={
                  overview?.lastActivityAt
                    ? formatDateTime(overview.lastActivityAt)
                    : "—"
                }
                hint="Registro"
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                  ...
                  <div className="mt-1 text-sm text-slate-700">
                    En “Respuestas” solo se muestra el{" "}
                    <span className="font-semibold">último intento</span>{" "}
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    <div className="mt-4">
                      ...
                      <div className="mt-2 h-2 w-full rounded-full bg-slate-200">
                        <div
                          ...
                          style={{
                            width: `${clampPct(overview?.completionPct)}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
                    ...
                    <div className="mt-2 text-sm text-slate-700">
                      Intentos totales:{" "}
                      <span className="font-extrabold">
                        {overview?.attemptsTotal ?? 0}
                      </span>
                    </div>
                    <div className="mt-2 text-sm text-slate-700">
                      Recompensas:{" "}
                      <span className="font-extrabold">
                        {overview?.xpTotal ?? 0} XP
                      </span>{" "}
                      y{" "}
                      <span className="font-extrabold">
                        {overview?.coinsTotal ?? 0} coins
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-slate-600">
                      Se muestra el payload del último intento por actividad (si
                      existe).
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-3">
                  {lastAttemptActivities.length === 0 && (
                    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-700">
                      ...
                      <div className="mt-1">
                        Este estudiante aún no tiene intentos con respuestas
                        registradas.
                      </div>
                    </div>
                  )}
                ...
                <div className="mt-4">
                  {insignias.length === 0 ? (
                    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-700">
                      ...
                      <div className="mt-1">
                        Aún no ha desbloqueado insignias.
                      </div>
                    </div>
                  ) : (
                    <div className="grid gap-3 md:grid-cols-3">
...
function KpiCard({ label, value, hint, accent, dark }) {
  return (
    <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
      ...
      <div
        className={`mt-2 text-2xl font-extrabold ${accent ?? "text-slate-900"}`}>
        {value}
      </div>
      {hint && (
        <div
          className={`mt-1 text-xs font-semibold ${dark ? "text-slate-800" : "text-slate-600"}`}>
          {hint}
        </div>
      )}
    </div>
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <MiniStat
          label="XP"
          value={`${a.reward_xp_total ?? 0}/${a.max_xp ?? 0}`}
        />
        <MiniStat
          label="Coins"
          value={`${a.reward_coins_total ?? 0}/${a.max_coins ?? 0}`}
        />
      </div>
