...
import { resolveInsignia } from "./helpers";
    ...
    return (
      <div className="grid grid-cols-3 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-16 rounded-xl border border-border bg-secondary/50" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Colección</div>
        <div className="text-xs font-bold text-foreground">{typeof count === "number" ? `${count} total` : ""}</div>
      </div>
        ...
        {insignias.slice(0, 9).map((ui) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-1 text-[10px] font-semibold text-muted-foreground">+{b?.value ?? 0}</div>
            </div>
          );
        })}
      </div>
